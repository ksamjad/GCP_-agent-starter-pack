"""BigQuery agent configured for the wmt-ade-agentspace-dev project."""
...
from bq_agent_app.metadata_utils import (
...
def run_python_analysis(code: str) -> dict[str, Any]:
    ...
    try:
        ...
        def compose_dashboard(
            ...
            *,
            ...
            """Create a Plotly dashboard from high-level chart specifications."""
    ...
    try:
        with contextlib.redirect_stdout(stdout_buffer):
            exec(  # noqa: S102
                compile(code, "<bq_agent_python_tool>", "exec"),
                safe_globals,
                sandbox_locals,
            )
    except Exception:
...
def plan_dashboard(
    ...
    return create_dashboard_plan(
        objective=objective, question=question, focus_tables=focus_tables
    )
...
BigQuery project from the US Central region. Speak with a professional yet
conversational British tone.

Before issuing SQL, lean on the curated metadata via the provided tools to
confirm relevant tables, columns, and row counts. When queries mention
Microsoft 365, mailboxes, or collaboration tooling, focus exclusively on the
`ms_graph` dataset. For workforce, employment, attrition, or hiring questions,
consult the `gt_wf` dataset straightaway. If context is unclear, call the
`recommend_dataset` tool to explain your routing choice.

Encourage rich insights with visual storytelling. Use the Python analysis tool
for deeper exploration, data wrangling, and to assemble dashboards with the
`compose_dashboard` helper or the `plan_dashboard` tool. Narrate your analytical
steps, reference the metadata consulted, and explain how stakeholders should
...
root_agent = Agent(
    ...
    name="wmt_bigquery_agent",
    description=(
        "British-flavoured analyst for the wmt-ade-agentspace-dev BigQuery project"
    ),
