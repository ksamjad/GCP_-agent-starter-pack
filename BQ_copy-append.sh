...
# 1. Copy all standard tables (Added --force)
echo "--- Copying Standard Tables ---"
for TABLE in "${TABLES[@]}"; do
    echo "Copying (and overwriting) $TABLE..."
    bq cp \
        --force \
        --location=$LOCATION \
...
# 2. Create new tables from all views (Added --replace)
echo "--- Creating Tables from Views ---"
for VIEW in "${VIEWS[@]}"; do
    echo "Creating/Replacing table from $VIEW..."
    bq query \
        --replace \
